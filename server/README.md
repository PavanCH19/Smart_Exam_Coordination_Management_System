...
Sequelize's `sync()` in `server.js` runs against the MySQL database named in `.env` (create the database itself first: `CREATE DATABASE smart_exam;`). Database behavior is configured with these Spring Boot-style environment properties:

| Property | Values | Default | Description |
|---|---|---|---|
| `DB_SHOW_SQL` | `true`, `false` | `false` | Enables or disables Sequelize SQL logging globally. |
| `DB_DDL_AUTO` | `create`, `update`, `none` | `none` | Maps to `sync({ force: true })`, `sync({ alter: true })`, or the default `sync()` behavior. |

Morgan logs HTTP requests globally. Development uses the `dev` format; production uses `combined`.
