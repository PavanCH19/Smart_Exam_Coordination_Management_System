...
const getAllDepartments = async (filters, page, limit) => {
    const { rows, count } = await departmentRepository.findAllDepartments(filters, page, limit);
...
const deleteDepartment = async (departmentId) => {
    ...
    const courseCount = await departmentRepository.countCoursesByDepartmentId(departmentId);

    if (courseCount > 0) {
        throw new ApiError(
            409,
            "Cannot delete this department because courses are assigned to it. Reassign or delete those courses first."
        );
    }
